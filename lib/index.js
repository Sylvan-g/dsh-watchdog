...
export function apply(ctx, config = {}) {
    ...
    ctx.on('agent/request', async (payload, next) => {
        ...
        const contextResult = await contextGuard.handleRequest(payload, async () => loopResult.seed ?? loopResult);
        ...
        if (loopResult.shouldAct && loopResult.actionLevel) {
            const actionResult = actionChain.execute(loopResult.diagnostic?.trigger ?? 'loop-detected', payload, contextResult.seed);
            // 动作链可能返回 patch
