 ...
 * 创建 tool-retry 守卫
 */
export function createToolRetryGuard(config) {
    const state = {
        retryCount: new Map(),
        fallbackMap: new Map(),
     ...
     */
    function registerFallback(toolName, fallbacks) {
     ...
     */
    function handleResult(ctx, result) {
        if (!result.isError) {
            // 成功则重置重试计数
     ...
     * 处理降级到备用工具
     */
    function handleFallback(ctx, result) {
...
//# sourceMappingURL=tool-retry.js.map