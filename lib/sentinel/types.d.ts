...
export declare function extractTextFromBlocks(blocks: ContentBlock[] | string | undefined): string;
/** 判断 ContentBlock[] 是否表示错误 */
export declare function isErrorContent(blocks: ContentBlock[] | undefined): boolean;
...
/** 默认配置 */
export declare const DEFAULT_CONFIG: WatchdogConfig;
/** watch() 的判定结果 */
...
export interface DiagnosticReport {
    ...
    trigger: string;
    /** 重复动作列表 */
    repeatedActions: Array<{
        signature: string;
        count: number;
    }>;
    /** 每步 token 消耗 */
    stepTokenUsage: Array<{
        step: number;
        inputTokens: number;
        outputTokens: number;
    }>;
...
export declare function extractAgent(payload: AgentRequestPayload): AgentLike | undefined;
/** 从 payload 中安全提取 session events */
export declare function extractEvents(payload: AgentRequestPayload): SessionEvent[];
