...
export function createStop() {
     ...
     * @returns 停止结果
     */
    function executeHardStop(agentCancel, sessionId, trigger = 'watchdog-loop-detected') {
     ...
     * @returns 停止结果 + 注入的消息
     */
    function executeSoftStop(agentSteer, sessionId, trigger = 'watchdog-loop-detected') {
        reportBuilder = new DiagnosticReportBuilder(sessionId)
            .setFinalState('CONFIRMED')
            .setTrigger(trigger)
