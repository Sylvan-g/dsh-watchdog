...
export class DiagnosticReportBuilder {
  ...
  private finalState: LoopState = 'NORMAL'
  private trigger = ''
  private repeatedActions: Array<{ signature: string; count: number }> = []
  private stepTokenUsage: Array<{ step: number; inputTokens: number; outputTokens: number }> = []
  private stalledSinceStep = -1
  private actionChain: Array<{ level: 'L1' | 'L2' | 'L3'; action: string; result: string; timestamp: number }> = []
  private contextGuardTriggers: Array<{ type: 'soft' | 'hard'; tokenCount: number; timestamp: number }> = []
  private toolRetryLog: Array<{ tool: string; attempt: number; result: 'success' | 'failed'; timestamp: number }> = []
  ...
  /** 设置重复动作 */
  setRepeatedActions(actions: Array<{ signature: string; count: number }>): this {
