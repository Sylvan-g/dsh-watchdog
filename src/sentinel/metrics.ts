 ...
 */
export function actionSignature(name: string, args: string): string {
  try {
 ...
 */
function normalizeArguments(obj: unknown): unknown {
  ...
  if (typeof obj === 'string') {
    ...
    // 绝对路径：整个字符串以 / 或 X:\ 开头
    if (/^(\/[^\s]*|[A-Z]:\\[^\s]*)/.test(obj)) return '<path>'
    // 长数字串（如文件 inode、时间戳毫秒）
    if (/^\d{10,}$/.test(obj)) return '<timestamp>'
    return obj
  }
  if (typeof obj === 'number') return '<num>'
  if (typeof obj === 'boolean') return obj
  if (Array.isArray(obj)) return obj.map(normalizeArguments)
 ...
 */
export function computeInfoGain(newContent: string, historyContents: string[]): number {
  if (!newContent || newContent.trim().length === 0) return 0
  if (historyContents.length === 0) return 1

  const newNgrams = charNgrams(newContent)
  if (newNgrams.size === 0) return 0
  ...
  for (const ng of newNgrams) {
    if (!historyNgrams.has(ng)) novelCount++
  }
 ...
 */
function extractToolResultText(event: SessionEvent): string {
 ...
 */
function extractTokenUsage(events: readonly SessionEvent[]): Array<{ inputTokens: number; outputTokens: number }> {
 ...
 */
function extractToolCalls(events: readonly SessionEvent[], windowK: number): SessionEvent[] {
  const toolCalls = events.filter((e): e is SessionEvent & { type: 'tool/call' } => e.type === 'tool/call')
 ...
 */
function extractToolResults(events: readonly SessionEvent[], windowK: number): SessionEvent[] {
  const toolResults = events.filter((e): e is SessionEvent & { type: 'tool/result' } => e.type === 'tool/result')
 ...
 * 计算动作去重率
 */
function computeDedupRatio(toolCalls: SessionEvent[]): { ratio: number; topSignatures: Array<{ signature: string; count: number }> } {
  if (toolCalls.length === 0) return { ratio: 0, topSignatures: [] }
 ...
 */
function computeWindowInfoGain(toolResults: SessionEvent[], allHistoryResults: SessionEvent[]): number {
  ...
  if (recentContents.length === 0) return 1
 ...
 */
function computeTokenSlope(events: readonly SessionEvent[], windowK: number): number {
  const allUsages = extractTokenUsage(events)
  const recentUsages = allUsages.slice(-windowK)

  if (recentUsages.length === 0) return 0

  let totalOutputTokens = 0
  let stepsWithOutput = 0
  for (const usage of recentUsages) {
    totalOutputTokens += usage.outputTokens
    if (usage.outputTokens > 0) stepsWithOutput++
  }

  if (stepsWithOutput === 0) return 0
 ...
 * @returns 进展度量结果
 */
export function computeMetrics(events: readonly SessionEvent[], config: LoopGuardConfig): ProgressMetrics {
